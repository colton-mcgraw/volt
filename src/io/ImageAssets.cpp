...
KeyValueManifest& imageManifest() {
  static KeyValueManifest manifest(std::filesystem::path("assets") / "images" / "manifest.txt");
  return manifest;
}

std::unordered_map<std::string, ImageEntryState>& imageEntryState() {
  static std::unordered_map<std::string, ImageEntryState> state;
  return state;
}
...
std::array<std::uint8_t, 4> fallbackColorFromKey(const std::string& key) {
  ...
  return {static_cast<std::uint8_t>((hash >> 16) & 0xFFu),
          ...
          static_cast<std::uint8_t>(hash & 0xFFu), 255u};
}
...
std::string resolveImagePathForKey(const std::string& textureKey) {
...
LoadedImageAsset loadImageAsset(const std::string& textureKey) {
...
bool hasImageAssetChanged(const std::string& textureKey) {
  if (textureKey.empty() || textureKey == "__white") {
    return false;
  }
  ...
  auto& state = imageEntryState();
  auto it = state.find(normalizedKey);
  ...
  if (it == state.end() || it->second.resolvedPath != newResolvedPath.value()) {
    ...
    const bool existed = (it != state.end());
